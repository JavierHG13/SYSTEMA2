...
function upload_questions(id_test)
{
    ...
    console.log("matricula alumno 1: "+matricula);
	especialidad= String($("#especialidad").val());
	//console.log(base_url+"SYSMATER/alumno/alumno");

	var url =  base_url+'sysmater/alumno/start_test/get_questions/' + id+'/'+matricula;
    //console.log(url);
	$.ajax({
		type: 'GET',
		url: url,
...
function _time_left(id,matricula) {
	var url = base_url + 'sysmater/alumno/start_test/get_time/' + matricula + '/' + id;
    //console.log("url time: "+url);
    ...
    if (data && data.length > 0 && data[0].time_left) {
        _change_time_left(data[0].time_left); 
        //console.log("tiempo: " + data[0].time_left);
    } else {
        console.warn('Sin datos de tiempo restante:', data);
    }
}, 
		error: function(xhr, status, error) {
			alert('Error al cargar tiempo restante: ' + error);
		}
	});
}
...
function start()
{
	...
	if (resp == 'F' || resp == 'L' || resp == 'C' || resp == 'D')
	{
		_save_response(recorre);
		contador = 1;
		recorre = 0;
...
{
	var id_reactivo_detail = String(array[r].id_reactivo_detail_sysmater);
	var id_reactivo_main = String(array[r].id_reactivo_main);
	var resp = String($('input:radio[name=radio]:checked').val());
	
	if (resp!="undefined") {
		console.log(array[r].Respuesta);
		//aconsole.log(resp);
		if (array[r].Respuesta!== resp)
		{
			array[r].Respuesta = resp;
			var url =  base_url+'/sysmater/alumno/start_test/save_response/' + matricula + '/' + id + '/' + id_reactivo_main+'/'+id_reactivo_detail + '/' + resp +'/'+ tiempo;
			$.ajax({
			type: 'GET',
			url: url,
			dataType:'json',
...
function finish_test(name)
{
	...
	if (pendientes==0)
	{
		...
		location = (base_url+"/SYSMATER/alumno/alumno");
	}
	else
	{
