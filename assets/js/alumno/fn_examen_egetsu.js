...
function upload_questions(id_test)
{
	...
	var url =  base_url+'/egetsu/alumno/start_test/get_questions/' + id+'/'+matricula;
	$.ajax({
		type: 'GET',
		url: url,
		dataType:'json',
...
function _time_left(id_test)
{
	...
	var url =  base_url+'/egetsu/alumno/start_test/get_time/' + matricula;
	$.ajax({
		type: 'GET',
		url: url,
		dataType:'json',
...
{
	var id_reactivo_detail = String(array[r].id_reactivo_detail_egetsu);
	...
	if (resp!="undefined") {
		...
		if (array[r].Respuesta!== resp)
		{
			...
			var url =  base_url+'/egetsu/alumno/start_test/save_response/' + matricula + '/' + id + '/' + id_reactivo_main+'/'+id_reactivo_detail + '/' + resp +'/'+ tiempo;
			$.ajax({
			type: 'GET',
			url: url,
			dataType:'json',
			...
			error: function()
			{
				alert('Error al guardar respuesta, regrese a la pregunta anterior y vuelva a intentar23.' + url);
			}
			});// end function ajax
		}// end if
	}
