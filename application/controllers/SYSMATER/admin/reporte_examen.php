...
class Reporte_examen extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->id_tipo == 1) {
			...
			$data['materias'] = $this->materia_model->lista_materias_examen_todos();
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($data['materias'] === NULL) {
					...
					$this->load->view("templates/views_sysmater/admin_views/admin_navbar_view");
					$this->load->view("templates/views_sysmater/admin_views/reporte_examen_view", $data);
					$this->load->view("templates/shared/footer_view");
				}
			} else {
				...
				if ($tipo == "excel" || $tipo == "pdf") {
					...
					$this->exportar($tipo, $materia, $examen);
				}
			}
		} else {
	...
	private function exportar($tipo, $materia, $examen)
	{
		$datosTodos = $this->reporte_model->obtener_resultados_examen_admin($materia, $examen);
		if ($datosTodos !== NULL){
			$datos = $datosTodos->result_array();
		} else {
			echo '<script type="text/javascript">';
			echo 'alert("No se encontraron registros");';
			echo 'window.location.href = "'.site_url('sysmater/admin/admin/reporte_examen').'";';
			echo '</script>';
			exit();
		}
	...
	public function get_data_examenes()
	{
		...
		if ($materia) {
			$query = $this->examen_model->lista_examenes_materia_todos($materia);
		} else {
			$query = NULL;
		}
