...
class Reporte_examen_parciales extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->id_tipo == 1) {
			...
			$this->validation();
			$data['periodos'] = $this->examen_model->lista_Periodos_Examenes();
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($data['periodos'] === NULL) {
					...
					$this->load->view("templates/views_sysmater/admin_views/admin_navbar_view");
					$this->load->view("templates/views_sysmater/admin_views/reporte_examen_parciales_view", $data);
					$this->load->view("templates/shared/footer_view");
				}
			} else {
				...
				if ($tipo == "excel" || $tipo == "pdf") {
					...
					$this->exportar($tipo, $materia, $periodo);
				}
			}
		} else {
	...
	private function exportar($tipo, $materia, $periodo)
	{
		$datosTodosAlumnos = $this->reporte_model->obtener_resultados_examen_parciales_admin($materia, $periodo);
		if ($datosTodosAlumnos !== NULL) {
			$datosTodosDocentes = $this->reporte_model->obtener_resultados_examen_parciales_docente_admin($materia, $periodo);
		} else {
			echo '<script type="text/javascript">';
			echo 'alert("No se encontraron registros");';
			echo 'window.location.href = "'.site_url('sysmater/admin/admin/reporte_examen_parciales').'";';
			echo '</script>';
			exit();
		}
	...
	private function crearPDF($datos, $datosTodosDocentes)
	{
		...
		if (!empty($datos) || $datos !== NULL) {
			...
			$filename = $primer_elemento['vchNomMateria'] . $primer_elemento['periodo'];
	...
	public function get_data_materias()
	{
		$periodo = $this->input->post('periodo');

		if ($periodo) {
			$query = $this->examen_model->lista_examenes_materia_periodo($periodo);
		} else {
			$query = NULL;
		}
