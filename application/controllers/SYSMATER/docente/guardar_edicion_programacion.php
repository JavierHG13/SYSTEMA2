...
class Guardar_edicion_programacion extends CI_Controller
{
	...
	public function index($id_examen,$grupo)
	{
        ...
        $data["aplicacion"] = $this->get_data_progamacion($id_examen, $grupo);
        ...
        if ($this->examen_model->existe_conflicto_horario($datos_filtro)) {
            ...
            $this->edita_programacion_examen($id_examen, $grupo);	
        }
		
	}

	private function edita_programacion_examen($id_examen, $grupo)
    {
        ...
        if (!$id_grupo) {
            $data['msg'] = "Error: Grupo no encontrado.";
			$this->load->view("templates/views_sysmater/docente_views/docente_navbar_view");
			$this->load->view("templates/views_sysmater/docente_views/edita_programacion_view", $data);
			$this->load->view("templates/shared/footer_view");
            return;
        ...
        if ($this->examen_model->actualiza_programa_examen($data)) {
            redirect("/sysmater/docente/docente/examenes_programados/");
        } else {
            redirect("/sysmater/docente/docente/edita_programacion/{$id_examen}/{$grupo}");
        }
    }
