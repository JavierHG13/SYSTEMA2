  ...
  <center>
    ...
    <div class="row" style="margin-bottom: 15px; display: flex; align-items: center; flex-wrap: wrap;">
        ...
        <div class="col-md-6" style="margin-bottom: 10px;">
            <form method="post" action="<?= site_url('/sysmater/director/alumnos/filtrar') ?>" class="form-inline" id="filtroForm">
            ...
            <?php if ($filtro_actual['periodo'] != $periodo_actual || $filtro_actual['cuatrimestre'] != 0 || $filtro_actual['grupo'] != 0): ?>
                <a href="<?= site_url('/sysmater/director/alumnos') ?>" class="btn btn-danger btn-sm" style="margin-left: 5px;">
            ...
            <tbody id="tablaCuerpo">
                <?php foreach ($alumnos->result() as $alumno): ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?= site_url('/sysmater/director/detalles_alumno/index/'.$alumno->vchMatricula) ?>" class='btn btn-success' alt="Ver alumno" style="margin-left: 10px;">
                                <span class="glyphicon glyphicon-check"></span> Ver Detalles
                            </a>
                        </td>
                    </tr>
                <?php endforeach; ?>
...
$(document).ready(function() {    
    $('#periodo').change(function() {
        ...
        $.ajax({
            url: '<?= site_url("/sysmater/director/alumnos/get_cuatrimestres_ajax") ?>',
            type: 'POST',
            dataType: 'json',
            data: { 
                periodo: periodo,
    ...
    $('#cuatrimestre').change(function() {
        ...
        $.ajax({
            url: '<?= site_url("/sysmater/director/alumnos/get_grupos_ajax") ?>',
            type: 'POST',
            dataType: 'json',
            data: { 
